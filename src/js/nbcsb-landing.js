...
jQuery(document).ready(function() {
    ...
    var date = moment().format().substr(0,10)
    for (var i = 1; i <= jQuery('.prize-wrap').length; i++){
        var gameDate = jQuery('.prize-wrap:nth-of-type(' + i + ')').attr('data-date');

        if (moment(gameDate).isBefore(date)){
            jQuery('.prize-wrap:nth-of-type(' + i + ') .prize').css('opacity', .4);
            jQuery('.prize-wrap:nth-of-type(' + i + ') .sold-out').show();
        }
    }
});